...
function convertUploadRequestsToUploadStateBranch(
  ...
  const uploadMetadata = files.reduce((uploadSoFar, file) => {
    if (
      !file.customMetadata?.annotations.length ||
      !template?.annotations.length
    ) {
      return {
        ...uploadSoFar,
    ...
    const setOfAnnotationIdsFromTemplate = new Set(
    ...
    return file.customMetadata.annotations.reduce(
      (
        ...
        if (
          !setOfAnnotationIdsFromTemplate.has(annotation.annotationId) &&
          annotationDefinition?.name !== AnnotationName.WELL &&
          annotationDefinition?.name !== AnnotationName.NOTES
        ) {
...
const viewUploadsLogic = createLogic({
  process: async (
    ...
    try {
      ...
      const metadataForFiles = await Promise.all(
        fileIds.map((fileId) =>
          Promise.all([
      ...
      const fileIdsAsFiles: UploadRequest[] = metadataForFiles.map(
        ([labkeyFileMetadata, customMetadata]) => ({
          ...labkeyFileMetadata,
          customMetadata,
          file: {
      ...
      const templateId = files.find((file) => !!file.customMetadata?.templateId)
        ?.customMetadata?.templateId;
      const basicTemplateInfo = templateId
        ? await mmsClient.getTemplate(templateId)
        : undefined;
